 ...
 * No section body spells the probe.
 */
...
export function gatedByOwner<K extends SectionKey>(module: SectionModule<K>): SectionModule<K> {
  ...
  const personal = async (
    ...
    const answer = await (ctx.read as unknown as OrgProbePort).org.probeAbsent(z.unknown(), {
      params: { org: ctx.repo.owner },
    });
    return "missing" in answer ? personalAccountNote(module, ctx.repo.owner, phase) : undefined;
  };
