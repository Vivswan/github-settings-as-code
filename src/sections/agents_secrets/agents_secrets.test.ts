/**
 * agents_secrets section tests: existence-based reconciliation (values can
...
describe("agents_secrets check mode", () => {
  test("declared-but-missing is drift; declared values get ONE cannot-verify note", async () => {
    ...
    const result = await agentsSecretsSection.run(ctx(api, true), [
      { name: "present", value: "$PRESENT_REF" },
      { name: "MISSING_ONE", value: "$REF_A" },
      { name: "MISSING_TWO", value: "$REF_B" },
    ]);
    expect(result.drift).toEqual([
      "agents_secrets[MISSING_ONE]: missing - declared in the settings file but not on the repo; apply will create it",
      "agents_secrets[MISSING_TWO]: missing - declared in the settings file but not on the repo; apply will create it",
    ]);
  ...
  test("an undeclared live secret is a keep-note by default, drift under undeclared: delete", async () => {
    ...
    const kept = await agentsSecretsSection.run(ctx(api, true), []);
    expect(kept.drift).toEqual([]);
    expect(kept.notes.join("\n")).toContain('Copilot agents secret "STALE" exists on the repo');

    const api2 = new MockApi({ [LIST]: listOf("STALE") });
    const deleted = await agentsSecretsSection.run(ctx(api2, true), {
      undeclared: "delete",
      entries: [],
    });
...
describe("agents_secrets apply mode", () => {
  test("seals and PUTs every declared secret: create and update, case-insensitively", async () => {
    ...
    const api = new MockApi({ [LIST]: listOf("EXISTING"), [PUBLIC_KEY]: KEY_ROUTE }).allowMutations(
      "PUT /repos/o/r/agents/secrets/EXISTING",
      "PUT /repos/o/r/agents/secrets/BRAND_NEW",
    );
    const result = await agentsSecretsSection.run(
    ...
    expect(puts.map((call) => call.path)).toEqual([
      "/repos/o/r/agents/secrets/EXISTING",
      "/repos/o/r/agents/secrets/BRAND_NEW",
    ]);
  ...
  test("re-PUTs a declared secret on every run even when the live list already has it", async () => {
    ...
    const api = new MockApi({ [LIST]: listOf("ROTATED"), [PUBLIC_KEY]: KEY_ROUTE }).allowMutations(
      "PUT /repos/o/r/agents/secrets/ROTATED",
    );
    const result = await agentsSecretsSection.run(ctx(api, false, { $R: "new-plaintext" }), [
  ...
  test("undeclared secrets: kept with a note by default, DELETED under the knob", async () => {
    ...
    const kept = await agentsSecretsSection.run(ctx(api, false), []);
    ...
    const api2 = new MockApi({ [LIST]: listOf("STALE") }).allowMutations(
      "DELETE /repos/o/r/agents/secrets/STALE",
    );
    const deleted = await agentsSecretsSection.run(ctx(api2, false), {
      undeclared: "delete",
      entries: [],
    });
  ...
  test("a hostile resolved value appears nowhere: not in results, paths, or payload text", async () => {
    ...
    const api = new MockApi({ [LIST]: listOf(), [PUBLIC_KEY]: KEY_ROUTE }).allowMutations(
      "PUT /repos/o/r/agents/secrets/EDGY",
    );
    const result = await agentsSecretsSection.run(ctx(api, false, { $H: hostile }), [
      { name: "EDGY", value: "$H" },
    ]);
    ...
    expect(put?.path).toBe("/repos/o/r/agents/secrets/EDGY");
  ...
  test("a failing PUT throws the API error without the plaintext in the message", async () => {
    const api = new MockApi({
      ...
      "PUT /repos/o/r/agents/secrets/DENIED_WRITE": {
        error: { status: 422, message: "Validation Failed", body: "" },
      },
    });
    let message = "";
    try {
      await agentsSecretsSection.run(ctx(api, false, { $V: "super-plain" }), [
  ...
  test("an unusable public key fails loudly before any PUT", async () => {
    ...
    await expect(
      agentsSecretsSection.run(ctx(api, false, { $V: "value" }), [{ name: "X", value: "$V" }]),
    ).rejects.toThrow(/no usable \{key_id, key\} pair/);
    expect(api.mutations()).toEqual([]);
  });
});

describe("agents_secrets validation", () => {
  test("case-insensitive duplicate names are rejected upfront", async () => {
    const api = new MockApi({});
    await expect(
      agentsSecretsSection.run(ctx(api, true), [
        { name: "token", value: "$A" },
        { name: "TOKEN", value: "$B" },
      ]),
    ).rejects.toThrow(/same agents_secrets entry/);
  ...
  // Public-key malformation prose (base64, X25519 length) is the shared
  // engine's and is pinned by the actions_secrets tests.
});
