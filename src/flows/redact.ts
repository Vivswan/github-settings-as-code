 ...
 * the report can name the private slug; a masked secret never reaches them, because GithubApi withholds every error body
 * and transport message of a secret-carrying request and every other run-path line is built from names, labels,
 * references, and constants (the e2e runner's checkReportLeaks sweeps each delivered report for the run's secrets).
 */
