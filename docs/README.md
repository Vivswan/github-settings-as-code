...
Contract tests pin the load-bearing claims to the code: the README's Sections and Inputs tables, [COVERAGE.md](../COVERAGE.md), and specific claims inside [undeclared policy](reference/undeclared-policy.md), [forward compatibility](reference/forward-compatibility.md), [private repositories](operate/private-repositories.md), [troubleshooting](operate/troubleshooting.md), and [check mode](operate/check-mode.md) - the defaults, counts, commands, and enumerations that must not drift. The rest is walkthrough prose: when a walkthrough and a pinned claim disagree, the pinned claim wins, so guides link to the claims rather than duplicating their exact wording.
