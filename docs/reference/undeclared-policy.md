...
The wrapped form names the policy explicitly. `entries` holds exactly what the array form would, and a wrapper that omits `_undeclared` behaves exactly like the plain array (the section default, or an inherited defaults-file policy, applies):
...
- A target's explicit `_undeclared` wins over the defaults'.
