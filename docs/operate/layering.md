...
- A section-worded error that prints the rejected value:
  ...
  - `branches` controls declared as a scalar where a mapping belongs, on a literal branch and a wildcard entry alike:
    `branches[0].protection.required_status_checks: required_status_checks must be a mapping of its keys (strict, then contexts or checks), or null to turn the requirement off`. This one names the key and prints nothing of the value.
