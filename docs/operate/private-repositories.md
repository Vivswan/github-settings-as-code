...
- **Each run:** the body is replaced; the issue opens when the target fails or drifts and closes when it is healthy. The run log announces the write with the issue number and the placeholder, never the slug: `report: updated issue #7 in private repository #1` (or `created`).
