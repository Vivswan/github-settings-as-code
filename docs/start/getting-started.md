...
The `@v2` pin <!-- x-release-please-major --> is the moving major tag, stable within its line; `@latest` is a moving tag on the packaged commit of the newest `main` source on the `build` branch (`main` itself is source-only), where breaking changes arrive unannounced, so keep production on the major pin; the tag exists from the first release cut on the `build` branch.
