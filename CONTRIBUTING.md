...
The release job runs downstream of the `all-green` gate, so releases and release-PR refreshes only happen from a green main. release-please only does version math, the changelog, the manifest and version pins, and the release PR; when that PR merges, the pipeline builds the bundle, commits it as a child of the merge commit, creates the `vX.Y.Z` tag once on that packaged commit, moves the moving major tag to it, and publishes the GitHub release (draft-first, with the assets attached) as the last step. The tag topology is unit-tested in test/scripts/release-pipeline.test.ts.
