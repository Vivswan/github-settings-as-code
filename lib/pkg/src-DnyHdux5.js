...
var GithubApi = class {
	...
	constructor(options) {
		...
		const underTestKnob = envKnob !== void 0;
		const throttleEnabled = !underTestKnob;
		const doNotRetry = Array.from({ length: 100 }, (_, i) => 400 + i).filter((s) => s !== 408 && !(underTestKnob && s === 429));
...
function throwFor(section, method, path, error, context) {
	...
	const effective = context?.op ? endpointPermission(section, context.op) : void 0;
	...
	const op = context?.op;
	const advice = op === void 0 ? void 0 : "outcomes" in op ? toleratedGraphqlErrors(op).filter((type) => error.graphqlTypes?.includes(type)).map((type) => op.outcomes[type]).filter((outcome) => outcome !== void 0).map((outcome) => outcome.charAt(0).toUpperCase() + outcome.slice(1)).join(". ") : op.hints?.[error.status];
...
const ENDPOINTS$15 = {
	...
	putProtection: {
		...
		denialHint: "a 404 answering \"Branch not found\" means the declared branch does not exist on the repo; create the branch, or remove it from the settings file",
...
*/
function isMissingBranch(error) {
...
async function planLiteralEntry(ctx, section, routed, branch, plan) {
	...
	if ("missing" in probe) {
		...
		if ("error" in branchProbe && isMissingBranch(branchProbe.error)) {
			plan.drift.push(`branches[${branch.name}]: declared in the settings file but the branch does not exist on the repo, so apply cannot protect it; create the branch, or remove it from the settings file`);
			return;
		}
		plan.ops.push({
			role: "putProtection",
...
//#endregion
export { stripNulls as $, toPublicView as A, prefixedIo as At, parseRecipient as B, concludeMerge as C, discoverRepos as Ct, capturingIo as D, isRateLimitError as Dt, PRIVATE_REPOS_POLICIES as E, isPermissionError as Et, MARKER_LABEL as F, REPO_RESULTS as G, describeProblem as H, MARKER_LABEL_CONFIG as I, skippedSectionKeys as J, preflightProbe as K, composeReport as L, applyMarkerInjection as M, resolveCentralTargets as Mt, openReportChannel as N, dedupeTargets as Nt, planRedaction as O, collectingIo as Ot, ISSUE_TITLE as P, parseRepoSlug as Pt, mergeLayers as Q, deliverArtifactReport as R, MERGE_RESULT as S, VISIBILITY_FILTERS as St, failRun as T, GithubApi as Tt, quoteList as U, RERUN_ADVICE as V, SectionSelection as W, worstOf as X, validateSettingsDoc as Y, describeOptOut as Z, runMulti as _, parseReposInput as _t, renderMergedYaml as a, readGating as at, createVisibilityResolver as b, DEFAULT_DISCOVERY_FILTERS as bt, readLayerFiles as c, writeGatedReads as ct, INPUT_DECLS as d, endpointPath as dt, SECTIONS as et, MERGE_INPUTS as f, DOCUMENT_DIRECTIVE_KEYS as ft, DEFAULT_SETTINGS_FILE as g, UNDECLARED_POLICY_SECTIONS as gt, parseConfig as h, SettingsFile as ht, checkRepository as i, denialPosture as it, PRIVATE_REPORT_CHANNELS as j, silentIo as jt, publicDetail as k, maskRegistry as kt, DEFAULT_PRIVATE_REPOS as l, grantFor as lt, MODES as m, SECTION_KEYS as mt, runMerge as n, allGraphqlOps as nt, validateSettings as o, sectionGrant as ot, MERGE_REJECTED_INPUTS as p, PROBOT_PARITY_KEYS as pt, runForRepo as q, applyRepository as r, sectionModule as rt, foldLayers as s, sectionOperations as st, runSingle as t, allEndpoints as tt, FILTER_INPUTS as u, endpointMethod as ut, parseSettingsDoc as v, AFFILIATIONS as vt, concludeRun as w, DEFAULT_API_VERSION as wt, getRepoFile as x, FORKS_FILTERS as xt, readSettingsFile as y, ARCHIVED_FILTERS as yt, encryptReport as z };
