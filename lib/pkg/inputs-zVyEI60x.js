...
function redactSecretPayloadSafe(payload) {
	...
	try {
		if (typeof payload !== "object" || payload === null) return payload === null || typeof payload === "string" || typeof payload === "boolean" || typeof payload === "number" && Number.isFinite(payload) ? ok({
			payload,
			traced: payload,
			carriesSecret: false
		}) : err(void 0);
		if (!isPlainJsonContainer(payload)) return err(describeNotPlain(new NotPlainData([], nonPlainKind(payload))));
		const normalized = normalizePlainData(payload);
		if (normalized instanceof NotPlainData) return err(describeNotPlain(normalized));
...
var GitHubApi = class {
	...
	async request(method, path, payload, options) {
		...
		if (secretScan.isErr()) return { failed: `${method} ${path} was not sent: ${secretScan.error ?? "its payload is not plain JSON data (a value carrying a function or exotic prototype)"}, so it could not be safely inspected for secret fields. Replace that value with a plain string in the settings file` };
		...
		const trace = (status) => {
			...
			this.trace.debug(`${method} ${safe.path} -> ${status} (${Date.now() - started}ms)` + (safe.redacted || payload === void 0 ? "" : marked ? ` payload: ${MARKED_PAYLOAD_TRACE}` : ` payload: ${JSON.stringify(secretScan.value.traced)}`));
		};
		try {
			const response = await this.octokit.request({
				...
				...payload === void 0 ? {} : { data: secretScan.value.payload }
			});
	...
	*/
	async tryGraphql(op, variables, slug, options) {
		...
		if (scan.isErr()) {
			const reason = scan.error ?? "its variables are not plain JSON data (a value carrying a function or exotic prototype)";
...
function parseMaterial(pattern, raw, rejection) {
	...
	return match === null ? err(rejection(raw)) : ok({
		algorithm: match[1] ?? "",
...
function validateSecretRef(value, source, label) {
	if (REFERENCE_RE.test(value)) {
		...
		if (source === "target") return err(`${label} uses the secret reference ${value} in a target-fetched settings file; references are honored only in operator-owned settings sources, so a target repository cannot read the operator's environment`);
		const reserved = RESERVED_REF_PREFIXES.find((prefix) => name.startsWith(prefix));
		if (reserved !== void 0) return err(`${label} references the reserved runner variable ${value} (${reserved}* is refused): workflow inputs and GitHub/runner context cannot be routed into settings values`);
		return ok({ name });
	}
	const embedded = value.match(EMBEDDED_REFERENCE_RE)?.[0];
	if (embedded !== void 0) return err(`${label} embeds ${embedded} without being a whole-value reference; it would otherwise ship verbatim as the secret. Make the entire value a single $NAME reference`);
	return err(`${label} carries a literal value, but settings files are committed plaintext - exactly what secret references exist to prevent. Set it to a whole-value $NAME reference and define NAME in the step's env block`);
}
...
function mintSecretReference(variable, label) {
	...
	if (checked.isErr()) throw new Error(`BUG: the snapshot minted a reference the settings file refuses: ${checked.error}`);
	return {
		variable,
		reference
	};
...
function declaredMaterial(title, key) {
	const parsed = parsePublicKey(key);
	if (parsed.isErr()) throw new Error(`BUG: deploy_keys[${title}] reached plan() with material the shape refuses (${parsed.error}); the shape refuses it first`);
...
function secretReferenceProblems(key, parsed, source) {
	...
	for (const { label, value } of module.secretValues?.(parsed) ?? []) {
		...
		if (checked.isErr()) problems.push(`${key}: ${checked.error}`);
	}
	return problems;
}
/**
	...
	if (!check) {
		...
		if (secretReferences.length > 0) {
			...
			for (const [key, names] of bySection) {
				...
				for (const plaintext of resolution.value.mask) mask.add(plaintext);
			}
