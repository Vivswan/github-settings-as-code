#!/usr/bin/env node
import { D as writeReplacing, Gt as maskRegistry, Lt as countNoun, P as failRun, Z as describeProblem, a as MERGE_ONLY_INPUTS, c as SNAPSHOT_INPUTS, d as parseConfig, f as parseSnapshotFileConfig, i as MERGE_INPUTS, j as readSettingsFile, l as SNAPSHOT_ONLY_INPUTS, lt as SECTIONS, p as snapshotFileDestination, qt as redactRanges, r as INPUT_DECLS, tt as skippedSectionKeys, yt as sectionGrant, zt as GitHubApi } from "./inputs-C8XeppNt.js";
import { a as snapshotRepository, o as validateSettings, s as executeRun } from "./src-C2cbAKM8.js";
...
function parseInitConfig(read, force, env) {
	return parseSnapshotFileConfig(read, env, "settings-file").map((cfg) => ({
		kind: "init",
		token: cfg.token,
...
function inputsForMode(mode) {
	...
	const modeOnly = [...MERGE_ONLY_INPUTS, ...SNAPSHOT_ONLY_INPUTS];
	const reads = (name) => {
		switch (mode) {
			case "merge": return MERGE_INPUTS.includes(name);
...
function buildProgram(options) {
	...
	init.option("--force", "Replace the settings file when it already exists; without it, init refuses").action(async function() {
		...
		const rendered = await parseInitConfig(read, values.force === true, host.env).match((cfg) => executeInit(cfg, io), async (problem) => failInit(io, problem, snapshotFileDestination(read, "settings-file")));
