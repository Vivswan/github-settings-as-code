...
function renderPath$1(path) {
	return path.map((step, i) => typeof step === "number" ? `[${step}]` : `${i === 0 ? "" : "."}${String(step)}`).join("");
}
/**
...
function ruleUnionError(issue) {
	...
	return (typeof type === "string" && KNOWN_RULE_TYPES.includes(type) ? known : unknown).map((sub) => sub.path.length === 0 ? sub.message : `${renderPath$1(sub.path)}: ${sub.message}`).join("; ");
}
...
function valueAt$1(record, path) {
	let node = record;
...
function identityClaimSites(identity, entry) {
	...
	const sites = [written === void 0 ? {
		name: valueAt$1(entry, pathOf(field)),
		field
	} : {
		name: written,
		field: renameKey
...
function nameOf(record, field) {
	const value = valueAt$1(record, pathOf(field));
...
function declaredSecrets(decl, write) {
	return (decl.secrets ?? []).filter((field) => typeof valueAt$1(write, pathOf(field)) === "string");
}
/** The write with every declared secret reference resolved, for the request body. */
function resolvedWrite(exec, write, fields) {
	const resolved = fields.reduce((out, field) => withValueAt(out, pathOf(field), exec.resolveSecret(String(valueAt$1(out, pathOf(field))))), write);
...
function comparison(decl, label, write, body, comparable) {
	const notes = [];
	const hidden = (decl.concealed?.(body) ?? []).filter((field) => valueAt$1(write, pathOf(field.field)) !== void 0);
...
function secretValuesFor(decl, declared) {
	...
	return secretValuesOf(declared, (entry) => fields.flatMap((field) => {
		const value = valueAt$1(entry, pathOf(field));
		if (typeof value !== "string") return [];
		const name = valueAt$1(entry, pathOf(decl.identity.field));
		return [{
...
function unreconcilable$1(envName, what) {
	return sectionFailure("live-shape", `environments: the deployment branch-policy list for environment "${envName}" returned a policy without ${what}, so it cannot be reconciled. Check the "api-version" input against the GitHub REST docs for this endpoint`);
}
function livePolicyId(policy, envName) {
	if (policy.id === void 0) return err(unreconcilable$1(envName, "an id"));
	return ok(String(policy.id));
}
function livePolicyName(policy, envName) {
	if (typeof policy.name !== "string") return err(unreconcilable$1(envName, "a name"));
...
function unreconcilable(envName, what) {
	return sectionFailure("live-shape", `environments: the deployment protection rule list for environment "${envName}" returned a rule without ${what}, so it cannot be reconciled. Check the "api-version" input against the GitHub REST docs for this endpoint`);
}
function liveRuleSlug(rule, envName) {
	const slug = rule.app?.slug;
	if (typeof slug !== "string") return err(unreconcilable(envName, "an app slug"));
	return ok(slug);
}
function liveRuleId(rule, envName) {
	if (typeof rule.id !== "number") return err(unreconcilable(envName, "a numeric id"));
		...
		if (!parsed.success) {
			...
			for (const issue of issues.slice(0, 5)) {
				...
				if (valueAt(declared, issue.path) === null && rewritesForNull(issue)) {
					problems.push(`${key}${path} has no empty state; write ${legalValues(issue)}`);
					continue;
				}
...
const INPUT_DECLS = {
	...
	"settings-file": {
		...
		summary: "Settings file path (single-repo mode); in `mode: render`, the ordered list of layers to fold, low to high",
		list: true
	},
	mode: {
		...
		default: "apply",
		summary: "`apply` mutates; `check` reports drift and exits 1 on any, making no settings changes (a private report may still be delivered); `render` folds the settings-file layers into rendered-file without touching GitHub; `snapshot` writes the live settings to snapshot-file or snapshot-dir"
	},
	"rendered-file": {
		...
		default: "",
		summary: "`mode: render` only (required there): where the rendered document is written, exactly what `apply` would run"
	},
	"snapshot-file": {
	...
	sections: {
		description: "Optional comma-separated allowlist of sections to process. apply, check, and snapshot only: mode: render writes every section its layers declare, so the allowlist belongs on the step that runs the rendered document and fails the render when set.",
		default: "",
		summary: "Comma-separated allowlist of sections to process (apply, check, and snapshot; rejected in `mode: render`)",
	...
	layering: {
		description: "mode: render only: replace, shallow, or deep (default), the run-wide default for how every list section's entries combine with the layers below them, each section by its own key (a label's name, a ruleset's name, a secret's name, ...). replace lets the higher list win wholesale; shallow unions the entries by key and swaps a same-key entry for the higher one; deep unions by key and merges a same-key pair field by field, a nested keyed list (a ruleset's rules, by type) unioning the same way. A layer's own _layering directive, at its top level or on a section's {entries} wrapper, overrides it per file or per section. Lists outside the list sections are replaced by the higher layer's. Fails when set in apply or check.",
		default: "",
		summary: "`mode: render` only: how every list section's entries combine across layers, by the section's key; `replace` lets the higher list win, `shallow` unions and swaps a same-key entry, `deep` unions and merges a same-key pair field by field; a layer's `_layering` overrides it",
		shownDefault: "`deep`"
	},
	undeclared: {
		...
		default: "",
		summary: "`keep` or `delete`: the fallback policy for every list that takes `_undeclared`, below a wrapper's and the file's own; unset, each list's default applies ([the undeclared policy](docs/reference/undeclared-policy.md))",
