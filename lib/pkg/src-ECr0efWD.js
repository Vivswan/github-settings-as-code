...
function snapshotUnsupportedNote(section) {
	if (planningReads(section).length === 0) return `${section.key}: GitHub exposes no read endpoint for this section, so there is nothing to snapshot; apply re-asserts the declared value on every run`;
...
function describeFiltersWithoutWildcard(problem) {
	...
	switch (problem.targets) {
		...
		case "snapshot-file": return `the discovery filter input(s) ${named} only apply to repos: "*" discovery, but this snapshot targets one repository. Set repos: "*" with snapshot-dir to discover repositories, or remove the filter input(s)`;
	}
}
...
function describeProblem(problem) {
	switch (problem.code) {
		...
		case "input-snapshot-only": return `the ${quoteList(problem.inputs)} input(s) only apply to mode: snapshot, but this run is in ${problem.mode} mode, so ${problem.inputs.length === 1 ? "it" : "they"} would never be used. Remove the input(s), or set mode: snapshot to write the live settings to a file`;
		case "input-rejected-in-snapshot": return `the ${quoteList(problem.inputs)} input(s) do not apply to mode: snapshot, which only reads the target repositories' live settings into snapshot-file or snapshot-dir: it applies no document, folds no layers, and delivers no report. Remove the input(s), or move them to the apply, check, or merge step they belong to`;
...
async function snapshotRepository$1(api, opts, io) {
	const outcomes = [];
	const document = {};
	let partial = false;
	let failed = false;
	for (const section of SECTIONS) {
		...
		for (const note of snapshot.notes) io.annotate("notice", underKey(section.key, note));
		...
		const verdict = validateSettingsDoc({ [section.key]: snapshot.value }, `the ${section.key} snapshot of ${opts.repo.slug}`, /* @__PURE__ */ new Set(), io);
		if (verdict.isErr()) {
	...
	const verdict = validateSettingsDoc(document, `the snapshot of ${opts.repo.slug}`, /* @__PURE__ */ new Set(), io);
...
*/
function renderSnapshotYaml(result, opts) {
	return `${[
		`# yaml-language-server: $schema=${opts.schemaUrl}`,
		`# Snapshot of ${result.repo} taken ${opts.timestamp}`,
		...result.outcomes.flatMap((outcome) => outcome.detail.flatMap((message) => message.split(/\r?\n/).map((line) => `# ${underKey(outcome.key, line)}`)))
	].join("\n")}\n${stringify(result.settings)}`;
}
//#endregion
...
function writeSnapshotSummary(io, view) {
	const lines = [
...
function writeSnapshotDirSummary(io, views, snapshotDir) {
	...
	for (const view of views) {
		lines.push("", `### ${markdownCell(view.display)} (${view.result})`, "", markdownCell(view.note), "");
		if (view.outcomes.length > 0) lines.push(...outcomeRows(view.outcomes));
	}
	io.summary(lines.join("\n"));
}
...
function parseSnapshotConfig(input, env) {
	return safeTry(function* () {
		...
		if (snapshotFile) {
			if (reposInput || reposDir) return err({ code: "input-snapshot-file-with-multi" });
...
function destinationCollision(cfg) {
	if (cfg.form === "file") return resolve(cfg.snapshotFile) === resolve(".github/settings.yml") ? err({
		code: "snapshot-file-is-settings-file",
		snapshotFile: cfg.snapshotFile,
		settingsFile: DEFAULT_SETTINGS_FILE
	}) : ok();
	if (cfg.reposDir && (isWithin(cfg.snapshotDir, cfg.reposDir) || isWithin(cfg.reposDir, cfg.snapshotDir))) return err({
...
function failedSnapshotTarget(message) {
	return {
		result: "failed",
		outcomes: [],
		note: message
...
*/
function snapshotView(channel, exposure, outcome, source) {
	...
	return {
		...
		...outcome.file === void 0 ? {} : { file: redacted ? REDACTED_DETAIL : outcome.file }
	};
}
/**
...
async function snapshotTarget(ctx) {
	...
	const result = await snapshotRepository$1(api, {
		repo,
		sections: cfg.sections,
		onMissingPermission: cfg.onMissingPermission
	}, channel.io);
	...
	try {
		writeReplacing(path, renderSnapshotYaml(result, {
			schemaUrl: SNAPSHOT_SCHEMA_URL,
			timestamp: ctx.timestamp
		}));
	} catch (error) {
		channel.io.annotate("error", `cannot write the snapshot to ${path}: ${String(error)}. Check that the "${ctx.pathInput}" input names a writable path`);
		return {
			result: "failed",
			outcomes: result.outcomes,
...
*/
function snapshotFilePath(dir, repo) {
	if ([repo.owner, repo.name].some((part) => part === "." || part === "..")) return { error: `the repository name "${repo.slug}" is not a GitHub owner/name (a "." or ".." segment), so it has no file under ${dir}` };
...
async function snapshotFile(api, cfg, io) {
	const opened = await openSingleRepoChannel(api, cfg, io);
	const outcome = await attempt(opened.channel, () => snapshotTarget({
		api,
		repo: cfg.repo,
		cfg,
	...
	return {
		form: "file",
		view: snapshotView(opened.channel, opened.exposure, outcome)
	};
}
...
function worstSnapshotResult(views) {
...
function concludeSnapshot(io, finished) {
	...
	if (finished.form === "file") writeSnapshotSummary(io, finished.view);
	else {
		writeSnapshotDirSummary(io, finished.views, finished.snapshotDir);
		io.output("repos-result", JSON.stringify(Object.fromEntries(finished.views.map((view) => [view.display, {
			result: view.result,
			source: view.source,
			skippedSections: view.outcomes.filter((o) => o.status === "skipped").map((o) => o.key)
		}]))));
	}
	io.output("skipped-sections", [...new Set(views.flatMap((view) => view.outcomes.filter((o) => o.status === "skipped").map((o) => o.key)))].join(","));
...
async function snapshotRepository(client, repo, options = {}) {
	...
	const log = options.io === void 0 ? collected.lines : [];
	if (result.result === "failed") return {
		...result,
		log
	};
...
async function snapshotRepositories(client, targets, options = {}) {
	const reports = [];
	for (const repo of targets) reports.push(await snapshotRepository(client, repo, options));
	return reports;
}
//#endregion
...
//#endregion
export { RERUN_ADVICE as $, readSettingsFile as A, AFFILIATIONS as At, publicDetail as B, collectingIo as Bt, SNAPSHOT_ONLY_INPUTS as C, endpointPath as Ct, resolveTargets as D, SettingsFile as Dt, DEFAULT_SETTINGS_FILE as E, SECTION_KEYS as Et, concludeRun as F, discoverRepos as Ft, ISSUE_TITLE as G, resolveCentralTargets as Gt, PRIVATE_REPORT_CHANNELS as H, prefixedIo as Ht, failRun as I, DEFAULT_API_VERSION as It, composeReport as J, MARKER_LABEL as K, dedupeTargets as Kt, PRIVATE_REPOS_POLICIES as L, GithubApi as Lt, getRepoFile as M, DEFAULT_DISCOVERY_FILTERS as Mt, MERGE_RESULT as N, FORKS_FILTERS as Nt, runMulti as O, UNDECLARED_POLICY_SECTIONS as Ot, concludeMerge as P, VISIBILITY_FILTERS as Pt, renderSnapshotYaml as Q, capturingIo as R, isPermissionError as Rt, SNAPSHOT_INPUTS as S, endpointMethod as St, parseConfig as T, PROBOT_PARITY_KEYS as Tt, applyMarkerInjection as U, redactRanges as Ut, toPublicView as V, maskRegistry as Vt, openReportChannel as W, silentIo as Wt, encryptReport as X, deliverArtifactReport as Y, parseRecipient as Z, INPUT_DECLS as _, readGating as _t, snapshotRepositories as a, runForRepo as at, MERGE_REJECTED_INPUTS as b, writeGatedReads as bt, SNAPSHOT_RESULTS as c, worstOf as ct, runSnapshot as d, stripNulls as dt, describeProblem as et, runSingle as f, SECTIONS as ft, FILTER_INPUTS as g, denialPosture as gt, DEFAULT_PRIVATE_REPOS as h, sectionModule as ht, renderMergedYaml as i, preflightProbe as it, createVisibilityResolver as j, ARCHIVED_FILTERS as jt, parseSettingsDoc as k, parseReposInput as kt, SNAPSHOT_SCHEMA_URL as l, describeOptOut as lt, readLayerFiles as m, allGraphqlOps as mt, applyRepository as n, SectionSelection as nt, snapshotRepository as o, skippedSectionKeys as ot, foldLayers as p, allEndpoints as pt, MARKER_LABEL_CONFIG as q, parseRepoSlug as qt, checkRepository as r, REPO_RESULTS as rt, validateSettings as s, validateSettingsDoc as st, runMerge as t, quoteList as tt, concludeSnapshot as u, mergeLayers as ut, MERGE_INPUTS as v, sectionGrant as vt, SNAPSHOT_REJECTED_INPUTS as w, DOCUMENT_DIRECTIVE_KEYS as wt, MODES as x, grantFor as xt, MERGE_ONLY_INPUTS as y, sectionOperations as yt, planRedaction as z, isRateLimitError as zt };
