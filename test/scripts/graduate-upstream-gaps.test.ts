...
`;
  ...
  test("an empty directory degrades to an empty GAPS with both exports intact", () => {
    expect(generateIndex([])).toBe(`${header}
const GAPS = [] as const;
${derivations}`);
  });

  test("emits one import and one GAPS element per gap file, in sorted order", () => {
    expect(
      generateIndex(["pages-https", "merge-queue"]),
    ).toBe(`${header}import { GAP as mergeQueue } from "./merge-queue.js";
